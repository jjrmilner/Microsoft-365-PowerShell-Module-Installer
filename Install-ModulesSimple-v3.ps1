...
.SYNOPSIS
    Simplified Microsoft 365 PowerShell Module Installer with JSON Configuration (v3)
...
.DESCRIPTION
    ...
    v3 changes:
      - Valid profiles are now read from the JSON config (no hard-coded list) so every defined
        profile - including iso27001 and devworkstation - is selectable from the menu and -Profile.
      - settings.defaultProfile / settings.defaultScope / settings.skipVersionCheck are now honoured.
      - -Profile renamed internally to -InstallProfile (aliased -Profile) so it no longer shadows
        the $PROFILE automatic variable.
      - New 'Workstation Setup' menu option and -Workstation switch (runs Setup-Workstation.ps1).
      - TLS 1.2 forced on Windows PowerShell 5.1; cleanup wildcard match fixed; restart path
        prefers $PSCommandPath; stray DEBUG line removed.

.PARAMETER ConfigFile
    Path to the JSON configuration file (default: modules-config.json)

.PARAMETER EnableServices
    Comma-separated list of service names to enable (overrides profile/JSON enabled settings)
    Example: "authentication,identity,exchange"

.PARAMETER InstallProfile
    Predefined profile to use. Valid values are read from the 'profiles' section of the JSON config
    (e.g. basic, security, iso27001, devworkstation, developer, enterprise).
    Aliased as -Profile for backwards compatibility.
...
.PARAMETER Silent
    Silent mode for automated deployment - auto-upgrades to PS7 and installs the configured
    default profile (settings.defaultProfile), or an explicit -Profile if supplied.

.PARAMETER Workstation
    Run the workstation bootstrap (Setup-Workstation.ps1) - installs dev tools (winget),
    VS Code extensions and environment, then the devworkstation module profile.
...
.EXAMPLE
    .\Install-ModulesSimple-v3.ps1 -Silent
    # Silent mode - auto-upgrades to PS7 and installs settings.defaultProfile

.EXAMPLE
    .\Install-ModulesSimple-v3.ps1 -Workstation
    # Full developer-workstation bootstrap (tools + extensions + env + modules)
#>
...
param(
    ...
    [Parameter(HelpMessage = "Predefined profile to use (values come from the JSON 'profiles' section)")]
    [Alias('Profile')]
    ...
    [Parameter(HelpMessage = "Silent mode for automated deployment - auto-upgrades to PS7 and installs the configured default profile")]
    [switch]$Silent,

    [Parameter(HelpMessage = "Run the workstation bootstrap (Setup-Workstation.ps1) instead of installing modules")]
    [switch]$Workstation
)
...
function Remove-AllModules {
    ...
    Write-ColorOutput "- Utility modules (ImportExcel, PSWriteOffice, etc.)" -Color $Colors.Info
    ...
    if ($confirmation -eq 'DELETE ALL MODULES') {
        ...
        # NOTE: Get-InstalledModule -Name does NOT support wildcards reliably; filter client-side.
        $allInstalled = Get-InstalledModule -ErrorAction SilentlyContinue
        foreach ($wildcardPattern in $wildcardSearches) {
            try {
                $foundModules = $allInstalled | Where-Object { $_.Name -like $wildcardPattern }
        ...
        foreach ($moduleName in $allModules) {
            try {
                # Skip literal wildcard patterns (defensive; resolved names are concrete)
                if ($moduleName -match '\*') {
                    continue
                }
...
function Show-ProfileMenu {
        ...
        foreach ($serviceName in $profileConfig.services) {
            ...
            if ($service) {
                foreach ($moduleProperty in $service.modules.PSObject.Properties) {
                    ...
                    $moduleEnabled = if ($null -ne $moduleConfig.enabled) { $moduleConfig.enabled } else { $true }
                    if ($moduleEnabled) {
                        $totalModules++
                    }
                }
    ...
    # Add workstation setup option
    $workstationIndex = $menuIndex + 1
    Write-ColorOutput "`n  [$workstationIndex] Workstation Setup (dev tools, VS Code, environment)" -Color $Colors.Success
    Write-ColorOutput "      Installs winget tools + VS Code extensions + git/gh/env, then the devworkstation module profile" -Color $Colors.Info
    Write-ColorOutput "      Runs Setup-Workstation.ps1 (reads workstation-config.json)" -Color $Colors.Progress

    # Add cleanup option
    $cleanupIndex = $menuIndex + 2
    ...
    Write-ColorOutput "  .\Install-ModulesSimple-v3.ps1 -Profile basic" -Color $Colors.Success
    Write-ColorOutput "  .\Install-ModulesSimple-v3.ps1 -EnableServices `"authentication,identity`"" -Color $Colors.Success
    Write-ColorOutput "  .\Install-ModulesSimple-v3.ps1 -Workstation" -Color $Colors.Success
    Write-ColorOutput $('-'*80) -Color $Colors.Header
    ...
    do {
        ...
        if ($selection -match '^\d+$') {
            ...
            } elseif ($selectionNum -eq $workstationIndex) {
                Write-ColorOutput "`n[+] Launching workstation setup..." -Color $Colors.Success
                $setup = Join-Path (Get-ScriptDirectory) 'Setup-Workstation.ps1'
                if (Test-Path $setup) {
                    & $setup
                    exit $LASTEXITCODE
                } else {
                    Write-ColorOutput "[ERROR] Setup-Workstation.ps1 not found next to this script" -Color $Colors.Error
                    exit 1
                }
...
function Install-PowerShell7 {
    Write-ColorOutput "`n$('='*80)" -Color $Colors.Header
    Write-ColorOutput "INSTALLING POWERSHELL 7" -Color $Colors.Header
    Write-ColorOutput $('='*80) -Color $Colors.Header

    Write-ColorOutput "`nAttempting to install PowerShell 7 using winget..." -Color $Colors.Info

    # Check if winget is available
    try {
        $wingetCmd = Get-Command winget -ErrorAction Stop
        Write-ColorOutput "  [+] winget detected - proceeding with installation..." -Color $Colors.Info
    } catch {
        Write-ColorOutput "  [ERROR] winget not available on this system" -Color $Colors.Error
        Write-ColorOutput "  [INFO] winget is required for automatic PowerShell 7 installation" -Color $Colors.Warning
        Write-ColorOutput "  [MANUAL] Please download from: https://github.com/PowerShell/PowerShell/releases" -Color $Colors.Info
        return $false
    }

    try {
        Write-ColorOutput "  [+] Installing PowerShell 7 via winget..." -Color $Colors.Progress
        Write-ColorOutput "  [INFO] This may take a few minutes..." -Color $Colors.Warning

        $wingetProcess = Start-Process -FilePath "winget" -ArgumentList "install", "Microsoft.PowerShell", "--silent", "--accept-package-agreements", "--accept-source-agreements" -Wait -PassThru -NoNewWindow

        if ($wingetProcess.ExitCode -eq 0) {
            Write-ColorOutput "  [SUCCESS] PowerShell 7 installed successfully via winget" -Color $Colors.Success
            Write-ColorOutput "  [INFO] Installation completed - PATH updated automatically" -Color $Colors.Info
            return $true
        } else {
            Write-ColorOutput "  [FAILED] winget installation failed (Exit Code: $($wingetProcess.ExitCode))" -Color $Colors.Error
            Write-ColorOutput "  [MANUAL] Please download from: https://github.com/PowerShell/PowerShell/releases" -Color $Colors.Info
            return $false
        }
    } catch {
        Write-ColorOutput "  [FAILED] Installation error: $($_.Exception.Message)" -Color $Colors.Error
        Write-ColorOutput "  [MANUAL] Please download from: https://github.com/PowerShell/PowerShell/releases" -Color $Colors.Info
        return $false
    }
}
    ...
    if ($currentPSVersion.Major -ge 7) {
        ...
        if ($ps7Path) {
            ...
            if ($restartPS7) {
                ...
                # Get the current script path and parameters (prefer $PSCommandPath; $MyInvocation.ScriptName is deprecated)
                $currentScript = if ($PSCommandPath) { $PSCommandPath } elseif ($MyInvocation.ScriptName) { $MyInvocation.ScriptName } else { $null }

                if ($currentScript) {
                    # Build parameter string for restart
                    $paramString = ""
                    if ($EnableServices) { $paramString += " -EnableServices `"$EnableServices`"" }
                    if ($InstallProfile) { $paramString += " -Profile $InstallProfile" }
            ...
            Write-ColorOutput "`nPowerShell 7 not found on system" -Color $Colors.Warning

            if ($Silent) {
                Write-ColorOutput "Silent mode: Attempting automatic PowerShell 7 installation..." -Color $Colors.Warning
                $installPS7 = $true
            } else {
                Write-ColorOutput "Would you like to install PowerShell 7 automatically? (Y/n): " -Color $Colors.Warning -NoNewline
                $response = Read-Host
                $installPS7 = $response -notmatch '^[Nn]'
            }

            if ($installPS7) {
                $installSuccess = Install-PowerShell7
                if ($installSuccess) {
                    Write-ColorOutput "`n[SUCCESS] PowerShell 7 installed successfully!" -Color $Colors.Success
                    Write-ColorOutput "Restarting script in PowerShell 7..." -Color $Colors.Info

                    # Get the current script path and parameters for restart
                    $currentScript = if ($PSCommandPath) { $PSCommandPath } elseif ($MyInvocation.ScriptName) { $MyInvocation.ScriptName } else { $null }
                    ...
                    if ($currentScript) {
                        ...
                        # Try to find the newly installed PowerShell 7
                        $ps7NewPath = $null
                        $ps7NewLocations = @(
                            "${env:ProgramFiles}\PowerShell\7\pwsh.exe",
                            "pwsh.exe"
                        ...
                        foreach ($location in $ps7NewLocations) {
                            if ($location -eq "pwsh.exe") {
                                try {
                                    $null = Get-Command pwsh -ErrorAction Stop
                                    $ps7NewPath = "pwsh"
                        ...
                        if ($ps7NewPath) {
                            if ($Silent) {
                                $process = Start-Process -FilePath $ps7NewPath -ArgumentList "-File `"$currentScript`"$paramString" -Wait -PassThru
                                exit $process.ExitCode
                            } else {
                                Start-Process -FilePath $ps7NewPath -ArgumentList "-File `"$currentScript`"$paramString" -Wait
                                exit 0
                            }
                        } else {
                            Write-ColorOutput "[WARNING] PowerShell 7 was installed but cannot be located for restart" -Color $Colors.Warning
                            Write-ColorOutput "Please restart this script manually in a new PowerShell 7 session" -Color $Colors.Info
                        }
                    }
                } else {
                    Write-ColorOutput "`n[FAILED] PowerShell 7 installation failed" -Color $Colors.Error
                    Write-ColorOutput "Manual download: https://github.com/PowerShell/PowerShell/releases" -Color $Colors.Info
                    Write-ColorOutput "Continuing with current PowerShell version..." -Color $Colors.Warning
                }
            } else {
                Write-ColorOutput "`nContinuing with Windows PowerShell (some features may be limited)" -Color $Colors.Warning
                Write-ColorOutput "Manual download: https://github.com/PowerShell/PowerShell/releases" -Color $Colors.Info
            }
        }
...
function Install-ModulesSafely {
    param(
        [hashtable]$ModulesToInstall,
        [string]$Scope = 'CurrentUser',
        [switch]$SkipVersionCheck
    )

    Write-ColorOutput "`nStarting module installation (scope: $Scope)..." -Color $Colors.Header
    ...
    foreach ($module in $ModulesToInstall.GetEnumerator()) {
        ...
        if (-not $Force) {
            try {
                ...
                if ($existing) {
                    ...
                    if ($module.Value -eq "latest" -and $SkipVersionCheck) {
                        # Version checking disabled in settings - keep whatever is installed
                        Write-ColorOutput "  [SKIP] Present; version-check disabled - keeping v$($existing.Version)" -Color $Colors.Info
                        $shouldSkip = $true
                    } elseif ($module.Value -eq "latest") {
        ...
        if (-not $moduleExists) {
            try {
                Write-ColorOutput "  [INSTALL] Installing from PowerShell Gallery ($Scope scope)..." -Color $Colors.Progress
                ...
                if ($installedModule) {
                    Write-ColorOutput "  [SUCCESS] Installed v$($installedModule.Version) in $Scope scope" -Color $Colors.Success
                } else {
                    Write-ColorOutput "  [SUCCESS] Installation completed in $Scope scope" -Color $Colors.Success
                }
...
function Show-FinalResults {
    ...
    if ($FixGraphVersions -or $failed -gt 0) {
        ...
        Write-ColorOutput "- For Graph module conflicts: keep ALL Microsoft.Graph.* on the SAME version" -Color $Colors.Info
...
try {
    Write-ColorOutput $('='*80) -Color $Colors.Header
    Write-ColorOutput "MICROSOFT 365 POWERSHELL MODULE INSTALLER - SIMPLIFIED (v3)" -Color $Colors.Header
    Write-ColorOutput "Configuration-driven module management" -Color $Colors.Info
    Write-ColorOutput $('='*80) -Color $Colors.Header

    # Ensure TLS 1.2 on Windows PowerShell 5.1 (PSGallery requires it; PS7 already defaults to TLS1.2+)
    if ($PSVersionTable.PSVersion.Major -lt 6) {
        try { [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor [Net.SecurityProtocolType]::Tls12 } catch { }
    }

    # -Workstation: delegate to the workstation bootstrap and exit
    if ($Workstation) {
        $setup = Join-Path (Get-ScriptDirectory) 'Setup-Workstation.ps1'
        if (Test-Path $setup) {
            & $setup
    ...
    $parametersProvided = $EnableServices -or $InstallProfile -or $Force -or $Interactive -or $FixGraphVersions -or $Silent

    if ($Silent) {
        # Honour settings.defaultProfile; respect an explicit -Profile if supplied
        if (-not $InstallProfile) {
            $InstallProfile = if ($config.settings.defaultProfile) { $config.settings.defaultProfile } else { 'enterprise' }
        }
        Write-ColorOutput "Silent mode: using profile '$InstallProfile'" -Color $Colors.Info
    ...
    # Validate profile if provided (valid profiles come from the config, NOT a hard-coded list)
    if ($InstallProfile) {
        $validProfiles = @($config.profiles.PSObject.Properties.Name)
        if ($InstallProfile -notin $validProfiles) {
            Write-ColorOutput "[ERROR] Invalid profile '$InstallProfile'. Valid profiles are: $($validProfiles -join ', ')" -Color $Colors.Error
            exit 1
        }
        Write-ColorOutput "Using profile: $InstallProfile" -Color $Colors.Info
    }

    # Get enabled modules
    $modulesToInstall = Get-EnabledModules -Config $config -EnableServicesOverride $EnableServices -ProfileName $InstallProfile

    if ($modulesToInstall.Count -eq 0) {
        Write-ColorOutput "`n[WARNING] No modules are enabled for installation" -Color $Colors.Warning
        Write-ColorOutput "Edit $ConfigFile to enable module categories or use -EnableServices parameter" -Color $Colors.Info
        exit 0
    }
    ...
    $installScope = if ($config.settings.defaultScope) { $config.settings.defaultScope } else { 'CurrentUser' }
    if ($installScope -eq 'AllUsers') {
        $isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
        if (-not $isAdmin) {
            Write-ColorOutput "[WARNING] defaultScope is 'AllUsers' but this session is not elevated - falling back to CurrentUser" -Color $Colors.Warning
            $installScope = 'CurrentUser'
        }
    }
    $skipVer = [bool]$config.settings.skipVersionCheck

    # Install modules
    $results = Install-ModulesSafely -ModulesToInstall $modulesToInstall -Scope $installScope -SkipVersionCheck:$skipVer
